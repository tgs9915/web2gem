...
const WARMUP = positiveInt(process.env.BENCH_WARMUP, Math.min(500, Math.floor(ITERATIONS / 4)));
...
const SOCKET_BODY_BYTES = positiveInt(process.env.BENCH_SOCKET_BODY_BYTES, 64 * 1024);
...
const STREAM_CUMULATIVE_STEPS = positiveInt(process.env.BENCH_STREAM_CUMULATIVE_STEPS, 160);
const STREAM_CUMULATIVE_STEP_CHARS = positiveInt(process.env.BENCH_STREAM_CUMULATIVE_STEP_CHARS, 256);
const STREAM_LONG_LINE_BYTES = positiveInt(process.env.BENCH_STREAM_LONG_LINE_BYTES, 64 * 1024);
const STREAM_LONG_LINE_CHUNK_BYTES = positiveInt(process.env.BENCH_STREAM_LONG_LINE_CHUNK_BYTES, 128);
const SOCKET_LONG_CHUNK_LINE_BYTES = positiveInt(process.env.BENCH_SOCKET_LONG_CHUNK_LINE_BYTES, 4096);
...
const socketTextSingleResponse = makeHttpResponseChunks(socketSingleBody, SOCKET_BODY_BYTES);
const socketTextMultiResponse = makeHttpResponseChunks(socketChunks, SOCKET_BODY_BYTES);
const uniqueItemsValue = makeUniqueCompositeValues(UNIQUE_ITEMS_COUNT);
const uniqueItemsRequirement = { type: "json_schema", schema: { type: "array", uniqueItems: true } };
const structuredPatternValue = { tag: "ok-ready", slug: "item-123", note: "alpha" };
...
const markdownRestorePlaceholderCount = countOccurrences(markdownRestoreMasked.text, "GEMINI_MD_PROTECTED_");
const largeJsonBody = JSON.stringify({ message: "x".repeat(128 * 1024), ok: true });
const longHeldToolCandidate = makeLongHeldToolCandidate(240 * 1024);
const cumulativeWrbLines = makeCumulativeWrbLines(STREAM_CUMULATIVE_STEPS, STREAM_CUMULATIVE_STEP_CHARS);
const longLineResponseChunks = makeLongLineStreamResponseChunks(STREAM_LONG_LINE_BYTES, STREAM_LONG_LINE_CHUNK_BYTES);
const socketLongChunkedLineResponse = makeSocketLongChunkedLineResponse(SOCKET_LONG_CHUNK_LINE_BYTES);
...
const cases = [
  ...
  { name: "route_health", fn: () => mod.default.fetch(new Request("https://worker.example/"), {}, {}) },
  { name: "route_models", fn: () => mod.default.fetch(new Request("https://worker.example/v1/models"), {}, {}) },
  { name: "json_invalid", fn: () => mod.default.fetch(new Request("https://worker.example/v1/chat/completions", { method: "POST", body: "{" }), {}, {}) },
  { name: "json_large_strict", fn: () => runReadLargeJsonStrict(), iterations: Math.min(ITERATIONS, 500), warmup: Math.min(WARMUP, 100) },
  { name: "json_oversized_reject", fn: () => mod.default.fetch(new Request("https://worker.example/v1/chat/completions", {
    method: "POST",
    headers: { "content-length": "95001" },
    body: "{}",
  }), {}, {}) },
  { name: "messages_to_prompt", fn: () => mod.messagesToPrompt(messages, toolBundle, "auto", toolBundle.defs, "", 1_000_000) },
  { name: "prepare_openai_context", fn: () => mod.prepareOpenAIGeminiContext(CFG, FAKE_PROVIDER, { model: "gemini-3.5-flash" }, messages, toolBundle, "auto", null, null) },
  { name: "parse_plain_long_false_positive", fn: () => mod.parseToolCalls(longFalsePositive, tools) },
  { name: "parse_valid_dsml", fn: () => mod.parseToolCalls(validDsml, tools) },
  { name: "stream_sieve_plain_chunks", fn: () => runPlainSieveChunks(longFalsePositive) },
  { name: "stream_sieve_held_tool", fn: () => runHeldToolCandidateChunks(longHeldToolCandidate), iterations: Math.min(ITERATIONS, 300), warmup: Math.min(WARMUP, 50) },
  { name: "stream_text_cumulative_deltas", fn: () => runStreamTextCumulativeDeltas(), iterations: Math.min(ITERATIONS, 120), warmup: Math.min(WARMUP, 20) },
  { name: "gemini_stream_long_line_chunks", fn: () => runGeminiStreamLongLineChunks(), iterations: Math.min(ITERATIONS, 80), warmup: Math.min(WARMUP, 10) },
  { name: "responses_small_delta_stream", fn: () => runResponsesSmallDeltaStream(), iterations: Math.min(ITERATIONS, 60), warmup: Math.min(WARMUP, 10) },
  { name: "google_small_delta_stream", fn: () => runGoogleSmallDeltaStream(), iterations: Math.min(ITERATIONS, 60), warmup: Math.min(WARMUP, 10) },
  { name: "sse_many_small_delta", fn: () => runSseChunks({ chunks: SSE_CHUNKS, delayMs: 0 }), iterations: Math.min(ITERATIONS, 400), warmup: Math.min(WARMUP, 100) },
  { name: "sse_slow_consumer", fn: () => runSseChunks({ chunks: SSE_SLOW_CHUNKS, delayMs: SSE_SLOW_DELAY_MS }), iterations: Math.min(ITERATIONS, 30), warmup: Math.min(WARMUP, 5) },
  { name: "socket_single_chunk_body", fn: () => runSocketSingleChunkBody(), iterations: Math.min(ITERATIONS, 1000), warmup: Math.min(WARMUP, 200) },
  { name: "socket_multi_chunk_body", fn: () => runSocketMultiChunkBody(), iterations: Math.min(ITERATIONS, 1000), warmup: Math.min(WARMUP, 200) },
  { name: "socket_text_single_chunk", fn: () => runSocketText(socketTextSingleResponse), iterations: Math.min(ITERATIONS, 400), warmup: Math.min(WARMUP, 100) },
  { name: "socket_text_multi_chunk", fn: () => runSocketText(socketTextMultiResponse), iterations: Math.min(ITERATIONS, 400), warmup: Math.min(WARMUP, 100) },
  { name: "socket_chunked_long_split_line", fn: () => runSocketText(socketLongChunkedLineResponse), iterations: Math.min(ITERATIONS, 80), warmup: Math.min(WARMUP, 10) },
  ...
  { name: "markdown_restore_many", fn: () => runMarkdownRestoreMany(), iterations: Math.min(ITERATIONS, 1000), warmup: Math.min(WARMUP, 200) },
];
...
const selectedCases = caseFilters.length
  ? cases.filter((item) => caseFilters.some((filter) => item.name.includes(filter)))
  : cases;
if (!selectedCases.length) {
  console.error(`No benchmark cases matched BENCH_CASES=${caseFilters.join(",")}`);
  process.exit(1);
}

console.log(`Benchmark iterations=${ITERATIONS} warmup=${WARMUP}`);
if (caseFilters.length) console.log(`Benchmark filters=${caseFilters.join(",")}`);
for (const item of selectedCases) {
  const result = await bench(item.fn, item);
  console.log(formatResult(item.name, result));
}
...
function formatDetails(value) {
  ...
  return Object.entries(value.__benchDetails)
    .map(([key, val]) => `${key}=${typeof val === "number" ? Math.round(val) : String(val)}`)
    .join(" ");
}
...
function makeLongFalsePositiveText(bytes) {
  const line = "ordinary prose: a < b, parameterless discussion, invoke is just a word, tool_calls appears as text.\n";
  let out = "";
...
async function runGeminiStreamLongLineChunks() {
  ...
  let chars = 0;
  try {
    for await (const delta of mod.generateStream(CFG, "bench", 1, 0, null, null)) {
      chunks += 1;
      chars += delta.length;
    }
  } finally {
...
async function runReadLargeJsonStrict() {
  const parsed = await mod.readJsonRequest(new Request("https://worker.example/", {
    method: "POST",
  ...
  return {
    __benchDetails: {
      ...
      ok: parsed.value && parsed.value.ok ? 1 : 0,
    },
  };
}
...
async function runSseChunks({ chunks, delayMs }) {
  ...
  const resp = mod.sseResponse(async (write) => {
    for (let i = 0; i < chunks; i++) {
      ...
      peakBufferedBytes = Math.max(peakBufferedBytes, producedBytes - consumedBytes);
      await write(frame);
      peakBufferedBytes = Math.max(peakBufferedBytes, producedBytes - consumedBytes);
    }
  });
...
function runSocketSingleChunkBody() {
  ...
  return {
    __benchDetails: {
      ...
      sameBuffer: out.buffer === socketSingleBody.buffer && out.byteOffset === socketSingleBody.byteOffset ? 1 : 0,
    },
  };
}
...
function runSocketMultiChunkBody() {
  ...
  return {
    __benchDetails: {
      totalBytes: out.length,
      sameBuffer: socketChunks.some((chunk) => out.buffer === chunk.buffer) ? 1 : 0,
    },
  };
}
...
async function runSocketText(responseChunks) {
  const resp = await mod.socketHttp(fakeSocketConnect(responseChunks), "https://example.test/text", { timeoutMs: 0 });
...
function makeHttpResponseChunks(bodyOrChunks, totalBytes) {
  ...
  if (bodyOrChunks instanceof Uint8Array) return [concatBytes(head, bodyOrChunks)];
...
function runUniqueItemsValidation() {
  const error = mod.validateStructuredOutputValue(uniqueItemsValue, uniqueItemsRequirement);
  return {
    __benchDetails: {
...
function runStructuredPatternValidation() {
  let error = "";
  for (let i = 0; i < 100; i++) {
    error = mod.validateStructuredOutputValue(structuredPatternValue, structuredPatternRequirement);
  }
  return {
    __benchDetails: {
      checks: 100,
...
function makeAppHtml(totalBytes, tokenBlock) {
  ...
  const fillerUnit = "app bootstrap payload: boq_assistant placeholder config data;\n";
  let filler = "";
...
function installFetchResponse(text) {
  ...
  globalThis.fetch = async () => new Response(new ReadableStream({
    pull(controller) {
      ...
      stats.pulls += 1;
      controller.enqueue(encoded.subarray(start, start + APP_HTML_CHUNK_BYTES));
    },
    cancel() {
      stats.canceled += 1;
    },
